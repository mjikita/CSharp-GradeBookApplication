...
namespace GradeBook.UserInterfaces
{
    public static class StartingUserInterface
    {
        ...
        public static void CreateCommand(string command)
        {
            ...
            var isWeightedString = parts[3].ToLower();
            var isWeighted = false;
            if (isWeightedString == "true" || isWeightedString == "false")
            {
                isWeighted = isWeightedString == "true";
            }
            else
            {
                Console.WriteLine($"{isWeightedString} is not a supported in gradebook, please try again");
                return;
            }
